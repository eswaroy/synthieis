...
# class DiabetesPredictionRequest(BaseModel):
#     """Request for diabetes and BP prediction."""
#     age: int = Field(..., ge=30, le=55, description="Patient age", example=42)
#     bmi: float = Field(..., ge=18.5, le=45.0, description="Body Mass Index", example=27.5)
#     average_rbs: float = Field(..., ge=100.0, le=400.0, description="Average RBS level", example=250.0)
#     hba1c: float = Field(..., ge=5.72, le=7.0, description="HbA1c percentage", example=6.5)
#     respiratory_rate: int = Field(..., ge=12, le=18, description="Respiratory rate", example=15)
#     heart_rate: int = Field(..., ge=70, le=100, description="Heart rate", example=80)
#     spo2: float = Field(..., ge=95.0, le=100.0, description="SpO2 percentage", example=98.0)
#     rbs_sequence: List[float] = Field(
#         ..., 
#         description="Hourly RBS readings (13 values from 6 AM to 6 PM)",
#         example=[220.0, 240.0, 260.0, 280.0, 270.0, 250.0, 260.0, 255.0, 245.0, 240.0, 235.0, 230.0, 225.0]
#     )
...
# class TrainingRequest(BaseModel):
#     """Request for model training - NO PATH INPUTS REQUIRED."""
#     model_config = ConfigDict(protected_namespaces=())
    
#     epochs: int = Field(default=50, ge=10, le=300, description="Number of training epochs", example=50)
#     batch_size: int = Field(default=32, ge=8, le=128, description="Training batch size", example=32)
#     learning_rate: float = Field(default=0.001, ge=0.0001, le=0.01, description="Learning rate", example=0.001)
...
# class DataGenerationRequest(BaseModel):
#     """Request for synthetic data generation."""
#     num_samples: int = Field(default=100, ge=1, le=1000, description="Number of samples to generate", example=100)
#     diabetes_ratio: float = Field(default=0.5, ge=0.0, le=1.0, description="Ratio of diabetic samples", example=0.5)
#     hypertension_ratio: float = Field(default=0.7, ge=0.0, le=1.0, description="Ratio of hypertensive samples", example=0.7)
